 ...
 * @param points output containing points on the unit sphere; coordinates
 *    are stored in column-major order
 * @param num_points number of points on the sphere
 * @param num_points_per_thread number of points to generate per thread

*/
__global__ void random_unit_sphere_points(curandState *state, float *points,
  ...
  for (int i = 0; i < num_points_per_thread; i++) {
    ...
    points[x_idx] = sinf(theta) * cosf(phi); // x
    points[y_idx] = sinf(theta) * sinf(phi); // y
    points[z_idx] = cosf(theta);             // z
  }
}
...
Eigen::MatrixXf random_sphere_points_cu(int num_points, unsigned long seed,
  ...
  float *d_points;
  CUDA_CALL(cudaMalloc(&d_points, num_points * 3 * sizeof(float)));

  // Generate the random points on the sphere
  random_unit_sphere_points<<<num_blocks, block_size>>>(
      d_states, d_points, num_points, num_points_per_thread);
  CUDA_CALL(cudaPeekAtLastError());
  ...
  CUDA_CALL(cudaMemcpy(points.data(), d_points, num_points * 3 * sizeof(float),
